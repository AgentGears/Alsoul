...
def test_f5b_reconciliation_upgrade_preserves_existing_effect_state(tmp_path, now):
    database = tmp_path / "f5b-reconciliation-upgrade.db"
    config = _alembic_config(database)
    command.upgrade(config, "0013_personal_calendar_create_effect")
    ...
    ids = FoundationBootstrapper(
        ...
        identity_namespace="f5b-reconciliation-migration",
    ...
    assert "personal_calendar_create_reconciliation_probe" in table_names


def test_f5b_reconciliation_downgrade_clears_v14_receipts_and_table(tmp_path, now):
    database = tmp_path / "f5b-reconciliation-downgrade.db"
    config = _alembic_config(database)
    command.upgrade(config, "head")

    scope = "ReconcilePersonalCalendarCreateUnknownEffect"
    engine = create_sqlite_engine(database)
    ...
    command.downgrade(config, "0013_personal_calendar_create_effect")
    ...
    assert remaining == 0
    assert "personal_calendar_create_reconciliation_probe" not in table_names
