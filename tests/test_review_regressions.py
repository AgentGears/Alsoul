...
def _bootstrap_memory(services, bootstrapper, now):
    ids = bootstrapper.bootstrap(identity_namespace="review", external_subject="u1")
...
def test_current_checked_rejects_capture_older_than_current_input(
    services, bootstrapper, now
):
    ids = _bootstrap_memory(services, bootstrapper, now)
...
def test_current_checked_rejects_explicitly_stale_valid_as_of(
    services, bootstrapper, now
):
    ids = _bootstrap_memory(services, bootstrapper, now)
    current, world = _prepare_current_world(
...
def test_investigation_relationship_must_belong_to_initiator(
    services, bootstrapper
):
    ids1 = bootstrapper.bootstrap(
        identity_namespace="review",
        external_subject="u1",
        surface_ref="surface-1",
        channel_ref="channel-1",
    )
    ids2 = bootstrapper.bootstrap(
        identity_namespace="review",
        external_subject="u2",
    ...
    with pytest.raises(DomainError) as exc:
        services.start_investigation(
            StartInvestigationCommand(
                uuid4(),
                ids2.companion_person_id,
                ids1.relationship_id,
...
def test_world_result_rejects_cross_investigation_contradiction(
    services, bootstrapper, now
):
    ids = _bootstrap_memory(services, bootstrapper, now)
    ...
    q1 = services.start_investigation(
        StartInvestigationCommand(
            uuid4(), ids.companion_person_id, ids.relationship_id, "q1"
        )
    )
    ...
    cap1 = services.record_observation_success(
        RecordObservationSuccessCommand(
            uuid4(),
            o1.observation_id,
            source1.source_identity,
            source1.requested_locator,
            source1.resolved_locator,
            source1.content,
            source1.captured_at,
        )
    )
    ...
    q2 = services.start_investigation(
        StartInvestigationCommand(
            uuid4(), ids.companion_person_id, ids.relationship_id, "q2"
        )
    )
    ...
    cap2 = services.record_observation_success(
        RecordObservationSuccessCommand(
            uuid4(),
            o2.observation_id,
            source2.source_identity,
            source2.requested_locator,
            source2.resolved_locator,
            source2.content,
            source2.captured_at,
        )
    )
    ...
    with pytest.raises(DomainError) as exc:
        services.admit_world_result(
            AdmitWorldResultCommand(
                ...
                24,
                (cap1.evidence_id,),
                contradiction_evidence_ids=(cap2.evidence_id,),
            )
        )
    assert exc.value.code == "WORLD_RESULT_WRONG_INVESTIGATION"


def test_adoption_rejects_text_not_matching_semantic_payload(
    services, bootstrapper, now
):
    ids = _bootstrap_memory(services, bootstrapper, now)
    current, world = _prepare_current_world(
