...
class ProgressivePresentationServices(ProgressivePresentationServicesV11):
    ...
    A canonical counterpart input that was durably admitted before terminal status was
    durably observed remains a valid interruption even if terminal reconciliation wins
    the attempt write fence before interruption handling runs. In that race no
    cancellation call is needed: the exact generation is already terminal, so the
    interruption is recorded with ``NOT_REQUIRED_TERMINAL`` and can still justify exact
    partial/zero Timeline truth. Input admitted only after terminal status observation is
    not retroactively promoted into an interruption.
    """
    ...
    def interrupt_attempt(
        ...
        with self.engine.connect() as conn:
            ...
            if replay:
                row = self._interruption(conn, UUID(replay["interruption_id"]))
            ...
            existing_id = existing["interruption_id"] if existing is not None else None
            existing_event_id = (
                existing["interrupting_event_id"] if existing is not None else None
            )
            existing_state = (
                existing["cancellation_request_state"] if existing is not None else None
            )

        if existing_state == "NOT_REQUIRED_TERMINAL":
            if existing_event_id != command.interrupting_event_id:
                fail(
            ...
            with self.engine.begin() as conn:
                ...
                if replay:
                    row = self._interruption(conn, UUID(replay["interruption_id"]))
                    return self._interruption_result(row)
                row = self._interruption(conn, existing_id)
                if (
                    row["interrupting_event_id"] != command.interrupting_event_id
                    or row["cancellation_request_state"] != "NOT_REQUIRED_TERMINAL"
                ):
                    fail(
                        "PROGRESSIVE_PRESENTATION_INTERRUPTION_CONFLICT",
        ...
        if attempt_state != "SETTLED":
            return super().interrupt_attempt(command)
        ...
        try:
            with self.engine.begin() as conn:
                ...
                if attempt["attempt_state"] != "SETTLED":
                    fail(
                        "PROGRESSIVE_PRESENTATION_INTERRUPTION_CONFLICT",
                        "presentation attempt state changed during interruption ordering",
                    )
            ...
            with self.engine.connect() as conn:
                ...
                if (
                    existing is None
                    or existing["interrupting_event_id"]
