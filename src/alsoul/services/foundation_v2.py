...
from alsoul.services.foundation import FoundationServices as FoundationServicesV1
from alsoul.services.foundation_base import FoundationServices as FoundationServicesCore
from alsoul.storage import schema


class FoundationServices(FoundationServicesV1):
    """Current F4 services with durable ConversationOpenLoop cognition selection."""
    ...
    def build_context_projection(
        ...
        if current_input is None or not requires_conversation_open_loop_context(
        ...
        if command.required_personal_predicates or command.required_world_result_ids:
            fail(
                ...
                "open-loop conversational context cannot also project personal or world propositions",
            )
        return self._build_open_loop_context_projection(command)
    ...
    def _build_open_loop_context_projection(
        ...
        with self.engine.begin() as conn:
            ...
            if not requires_conversation_open_loop_context(input_event["content_text"]):
                fail(
                    "CONVERSATION_OPEN_LOOP_PROJECTION_INVALID",
            ...
            loop = self._select_current_decision_loop(
                conn,
                ...
                "conversation_open_loop_items": [
                    {
                        ...
                        "opened_by_event_id": str(loop["opened_by_event_id"]),
    ...
    @staticmethod
    def _select_current_decision_loop(conn, *, relationship_id: UUID):
        rows = conn.execute(
    ...
    def render_provider_context(self, projection_id: UUID) -> dict[str, Any]:
        ...
        # Bypass the v1 bounded-adjacency renderer: open-loop context is relationship-
        # scoped and intentionally not labelled as immediate prior Timeline context.
